...
The following graphs and metrics aim at observing certain patterns between variables to propose relevant hypothesis testing.

##Metrics
```{r Exploratory Data, echo=FALSE}
#Exploration of potential relationships between variables
OverallLenDose <- cor(ToothGrowth$len,ToothGrowth$dose)

OJ <- subset(ToothGrowth,ToothGrowth$supp =="OJ")
VC <- subset(ToothGrowth,ToothGrowth$supp =="VC")

OJLenDose <- cor(OJ$len,OJ$dose)
VCLenDose <- cor(VC$len,VC$dose)
```

In a first instance, the linear relationship between tooth growth in guinea pigs and the dose of ascorbic acid appears to be strong, with a correlation of `r round(OverallLenDose,2)`. This linear relationship remains strong, and even gets stronger, within the delivery methods, with a correlation of `r round(OJLenDose,2)` and `r round(VCLenDose,2)` for orange juice and ascorbic acid respectively.
...
From this graph, the overall positive aspect of the relationship between the dose and the length of the odontoblast can be fully appreciated. By it's overall look, the ascorbic acid (*VC*) data displays a stronger linearity than the orange juice (*OJ*).
...
This graph is meant to compare both delivery method and showcase any significant difference between both.
...
Finally, this graph intends to portray the relationship between the growth of odontoblast in guinea pigs, with no distinction between delivery method.
...
```{r Alpha, echo=FALSE}
#Setting the alpha in order to establish the confidence level
alpha <- 0.05
```

The Alpha value selected for this experiment is `r alpha`, for a 95% confidence level.

###Supply Method

####Hypothesis Formulation and Presentation

Ascorbic acid delivery generates more odontoblast growth than orange juice delivery.

Ascorbic acid supply = $VC$
Orange Juice delivery = $OJ$
...
Hypo1 <- t.test(OJ$len,VC$len,paired=FALSE,conf.level=1-alpha)

#Establishing the rejection regions for the t-tests
RejRegionHypo1 <- qt(1-alpha/2,df=Hypo1$parameter)*c(-1,1)
```
...
#Testing for any statistical difference between them
Hypo2.1 <- t.test(Dose1$len,Dose0.5$len,paired=FALSE,conf.level=1-alpha)
Hypo2.2 <- t.test(Dose2$len,Dose1$len,paired=FALSE,conf.level=1-alpha)
Hypo2.3 <- t.test(Dose2$len,Dose0.5$len,paired=FALSE,conf.level=1-alpha)

#Establishing the rejection regions for the t-tests
RejRegionHypo2.1 <- qt(1-alpha/2,df=Hypo2.1$parameter)*c(-1,1)
RejRegionHypo2.2 <- qt(1-alpha/2,df=Hypo2.2$parameter)*c(-1,1)
RejRegionHypo2.3 <- qt(1-alpha/2,df=Hypo2.3$parameter)*c(-1,1)
```
...
```{r Summary Added}
#Some more summaries of the data
head(ToothGrowth)
str(ToothGrowth)
```

```{r Exploratory Data, eval=FALSE}
```
...
#Annex B - Distribution Graph
```{r Graph 1, message=FALSE}
## Graph 1 - Distribution of Odontoblast Length
